...
for(i in 1:(nrow(liste0))){
  ...
  res_k<-liste0[i,]
  res_k$k=0
  tmp_k<-data.table(matrix(nrow=0,ncol=3))
  colnames(tmp_k)=c("x","y","k")
  k=1
    while (k<10&nrow(res_k)!=0){
       nb_k1<-nrow(res_k)
      
      for (j in 1:nb_k1){
      tmp_k=rbind(tmp_k,next_k(res_k[j]$x,res_k[j]$y,k))
      }
      k=k+1
      max=max+1
...
# chercher la direction avec n+?+1
  #puis n+?+2 n+??+2
#jusqu'à n=0
