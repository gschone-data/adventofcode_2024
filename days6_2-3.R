...
positions_l<-positions[-vec,]

pos_obst=data.table(matrix(nrow=0,ncol=3))
colnames(pos_obst)=c("x","y","dir")
nb_loop=0
for (k in 1:(nrow(posistions_l)-1)){
  print(k)
  data2<-data
  obst<-posistions_l[k+1,]
  substr(data2$V1[as.integer(obst[1])],as.integer(obst[2]),as.integer(obst[2]))="#"
  p<-as.character(posistions_l[k,])
  temp<-replicate(4800-k,proj())
  if(test_out(p)==F){
        nb_loop=nb_loop+1
        print(nb_loop)
        pos_obst=rbind(pos_obst,as.list(obst))
